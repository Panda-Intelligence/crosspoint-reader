...
LANG_ABBREVIATIONS = {
    ...
    "中文": "ZH", "chinese": "ZH",
    "chinese (simplified) 简体中文": "ZHC",
    "chinese (traditional) 繁體中文": "ZHT",
    "日本語": "JA", "japanese": "JA",
    "한국어": "KO", "korean": "KO",
...
def generate_keys_header(
    ...
    # (Chinese first, then English, then by language code alphabetically)
    priority_codes = ["ZH_CN", "ZH_TW", "EN"]
    priority_indices = [languages.index(code) for code in priority_codes]
    rest = sorted(
        (i for i in range(len(languages)) if i not in priority_indices),
        key=lambda i: languages[i],
    )
    sorted_indices = priority_indices + rest
    comment_names = ", ".join(language_names[i] for i in sorted_indices)
    lines.append("// Sorted language indices for display (auto-generated by gen_i18n.py)")
