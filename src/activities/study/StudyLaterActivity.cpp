...
void StudyLaterActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_STUDY_LATER_CARDS));
  ...
  if (cards.empty()) {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, cardY + 18, tr(STR_STUDY_NO_LATER_CARDS), true, EpdFontFamily::BOLD);
    renderer.drawLine(pad + 18, cardY + 48, pageWidth - pad - 18, cardY + 48, true);
    renderer.drawCenteredText(SMALL_FONT_ID, cardY + 66, tr(STR_STUDY_USE_LATER_DURING_STUDY));
    renderer.drawCenteredText(SMALL_FONT_ID, cardY + 94, tr(STR_STUDY_POSTPONED_CARDS_STAY_HERE));
  } else {
    std::vector<std::string> decks;
    ...
    if (showingBack) {
      renderer.drawCenteredText(UI_10_FONT_ID, contentBottom - 30, tr(STR_STUDY_CONFIRM_MARKS_HANDLED));
    } else {
      char progress[80];
      snprintf(progress, sizeof(progress), tr(STR_STUDY_IN_DECK_LATER_TOTAL_FORMAT),
