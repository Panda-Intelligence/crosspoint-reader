...
parser.add_argument("--pack-output", dest="pack_output", action="store", help="Write binary font pack to this path.")
...
if args.pack_output:
    ...
    with open(args.pack_output, "wb") as f:
        header = struct.pack(
            ...
            2,
            flags,
            len(compressed_bitmap_data) if compress else len(glyph_data),
            len(glyph_props),
            len(intervals),
            len(compressed_groups) if compress else 0,
            len(kern_left_classes),
            len(kern_right_classes),
            len(kern_matrix),
            len(ligature_pairs),
            norm_ceil(face.size.height),
            norm_ceil(face.size.ascender),
            norm_floor(face.size.descender),
            1 if is2Bit else 0,
            kern_left_class_count,
            kern_right_class_count,
        )
        f.write(header)
        f.write(bytes(compressed_bitmap_data) if compress else bytes(glyph_data))
        ...
        if compress:
            ...
            for compressed, uncompressed_size, count, first_idx in compressed_groups:
                f.write(struct.pack("<IIIHHI", compressed_offset, len(compressed), uncompressed_size, count, 0, first_idx))
                compressed_offset += len(compressed)
